...
def extractFileNameMetadata(fileName):
    ...
    fileNamePattern = re.compile(r'Timepoint_(?P<timePoint>\d+)\\(?P<date>\d+)\-(?P<experimentName>.*)\-p(?P<plate>\d+)\-(?P<hps>\d+)hps_(?P<wellID>\w\d\d)_w(?P<channel>\d+)\.TIF', re.IGNORECASE)
