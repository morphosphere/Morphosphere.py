...
for image in range(0,len(dataTable),1):
    ...
    channelPattern = re.compile('1|3|5')
    #rowPattern = re.compile('[A-P]') ### use rowPattern.match(dataTable[image,3])
    #dpsPattern = re.compile('\d+') ### use dpsPattern.match(dataTable[image,6]
    platePattern = re.compile('4')
    if channelPattern.match(dataTable[image,9]) and columnPattern.match(dataTable[image,4]) and platePattern.match(dataTable[image,2]):
        dataTable[image, 12] = 1
        dataTable[image, 10] = 1 # is TL
...
#         if numpy.max(cv2.mean(imageCropped)) > 3000:
...
for currentImage in range(0,len(dataTable),1):
    ...
    if dataTable[currentImage,12] == 1 and dataTable[currentImage,10] == 1:
        ...
        if dataTable[currentImage,11] == 0:
            ...
            # consider only central image region #CUSTOMIZE, SHOULD INCLUDE ENTIRE CLUMP
            imageCropped = imageToAnalyze[650:1510, 650:1510]
            sobelx = cv2.Sobel(imageCropped, cv2.CV_64F, 1, 0, ksize=5)
            sobely = cv2.Sobel(imageCropped, cv2.CV_64F, 0, 1, ksize=5)
