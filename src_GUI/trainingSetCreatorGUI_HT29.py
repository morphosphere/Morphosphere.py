...
analysisTitle ='20160512_HT29complete_strict_focused'
exportTitle = '20160512_HT29complete_strict'
extension = "TIF"
...
outputDirectory = 'N:\\Fanny_Georgi\\1-12_TumorRemission\\20160126-1-12-4_Spheroid_Complete_Analysis'
np.savetxt(outputDirectory + '\\' + exportTitle + selectedTitle +'.csv', dataTableSelected, fmt='%5s', delimiter=',', header=csvHeader )
...
class MyApp(QtGui.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.skipButton.clicked.connect(self.skipButton_clicked)
    ...
    def class1Button_clicked(self):
        ...
        self.dataTableSelected[self.imageCounter, 13] = 1
    ...
    def skipButton_clicked(self):
        print 'Image ' + self.dataTableSelected[self.imageCounter, 1] + ' was skipped.'
    ...
    def backButton_clicked(self):
        print 'Classification of image ' + self.dataTableSelected[self.imageCounter-1, 1] + ' is undone. Please assign it to a class again'

        self.dataTableSelected[self.imageCounter-1, 11] = 1
        self.dataTableSelected[self.imageCounter-1, 14] = 0
        self.dataTableSelected[self.imageCounter-1, 15] = 0
    ...
    def saveButton_clicked(self):
        print 'MorphoSphere training set state was saved to ' + outputDirectory + '. To terminate MorphoSphere, press "Cancel".'
        ...
        # Paste images with flagFocus = 1 in new array, so excluded images will be discarded
        dataTableClassified = np.zeros((1, 21), dtype=object)
        dataTableTraining = np.zeros((1, 21), dtype=object)
        for currentRow in range(0, len(self.dataTableSelected), 1):
            if self.dataTableSelected[currentRow, 11] == '1':
                ...
                dataTableTraining = np.vstack([dataTableClassified, newRow])
        # Delete first empty row
        dataTableClassified = np.delete(dataTableClassified, (0), axis=0)
        dataTableTraining = np.delete(dataTableClassified, (0), axis=0)

        np.savetxt(outputDirectory + '\\' + exportTitle + classifiedTitle + '.csv', dataTableClassified, fmt='%5s',delimiter=',', header=csvHeader)
        np.savetxt(outputDirectory + '\\' + exportTitle + '_training' + '.csv', dataTableTraining, fmt='%5s',delimiter=',', header=csvHeader)
